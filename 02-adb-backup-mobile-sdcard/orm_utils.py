...
def _member_inject(obj, *args, **kwargs):
    ...
    obj_type = type(obj)
    obj_dict = obj_type.__dict__
    if '_member_inject_field_cache' in obj_dict:
        fields = obj_dict['_member_inject_field_cache']
    else:
        fields = obj_type.__FIELDS__
        fields = [x.field_name for x in fields if type(x) == TableFieldDescriptor]
        setattr(obj_type, '_member_inject_field_cache', fields)
    set_fields = set()
    # check arguments
    ...
    not_null_fields = set([x.field_name for x in obj_type.__FIELDS__ if
                           type(x) == TableFieldDescriptor and x.not_null and x.default is None])
...
class TableFieldDescriptor(AbstractTableDescriptor):
    def __init__(self, field_name: str, field_type: str, not_null: bool = False, unique: bool = False,
                 primary_key: bool = False, auto_increment: bool = False, default: Optional[str] = None):
