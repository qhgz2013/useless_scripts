...
def _create_table_not_exists(sql_stmt, cursor):
    match = re.search(create_table_stmt, sql_stmt)
    if match is None:
        raise ValueError('Invalid create table statement: %s' % sql_stmt)
    cursor.execute("select count(1) from sqlite_master where name = ? and type = 'table'", (match.group('table_name'),))
...
class GenericSqlAccessor:
    def __init__(self, sql_generator: sql_autogenerator.AbstractSqlStatementGenerator):
        self._generator = sql_autogenerator
...
class SqliteAccessor:
    def __init__(self, sqlite_path: str, ensure_thread_safe: bool = True):
        ...
        self._generator = sql_autogenerator.SqliteSqlStatementGenerator()
        self._check_tables()
    ...
    def _create_table_dependency_order(self, cursor, entity_class):
        ...
        cursor.execute("select count(1) from sqlite_master where name = ? and type = 'table'",
                       (entity_class.__TABLE_NAME__,))
        if cursor.fetchone()[0] == 0:
