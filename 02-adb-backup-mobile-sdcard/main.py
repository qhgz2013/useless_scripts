...
def main():
    ...
    parser.add_argument("--action", choices=['sync_local', 'sync_remote', 'map_fs', 'compress_db', 'restore_db'],
                        help='choose action, sync_local: sync from local to remote, sync_remote: sync from remote to'
                             ' local, map_fs: map objects to database', dest='action', required=True)
    ...
    parser.add_argument('fs_or_remote_path', help='remote path when syncing or fs path when mapping', type=str, nargs='?')
